...
func (s *Store) writeProcessTx(tx *sql.Tx, sample collector.Sample, data collector.ProcessData) error {
	...
	metricStmt, err := tx.Prepare(`INSERT INTO process_metrics
		(ts, pid, start_time, state, cpu_user_pct, cpu_system_pct, rss_bytes, num_fds, num_threads, read_bytes_sec, write_bytes_sec, net_rx_bytes_sec, net_tx_bytes_sec)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
	if err != nil {
	...
	for _, p := range data.Processes {
		...
		if _, err := metricStmt.Exec(
			...
			p.ReadBytesSec, p.WriteBytesSec, p.RxBytesSec, p.TxBytesSec,
		); err != nil {
