...
internal/collector/ — metric collectors (CPU, memory, disk, network, ECC, temperature, power, process)
internal/store/     — writes collected samples to DuckDB; archive.go handles Parquet export
internal/alert/     — alert engine (threshold, predictive, variance rules; DB-stored; webhook delivery)
internal/api/       — HTTP API over unix socket + optional TCP (status, alerts, config, history), JSON serialization
internal/tui/       — bubbletea views (dashboard, cpu, memory, disk, network, temperature, power, process, alerts)
...
- **TUI views**: each view is a standalone render function returning a string. Views are switched via number keys (1–N), Tab/Shift+Tab, or arrow keys. CPU, memory, disk, temperature, power, and process views show a historical chart below live data, with time range cycling via `<`/`>`. All content is wrapped in bordered panels via `renderPanel()`. The alerts view has two panels: a rules list (with cursor navigation, toggle, delete) and a fired alerts table. The temperature and power views have per-sensor/zone toggle selection (j/k to navigate, space to toggle, a for all) controlling which items appear on the history chart; selections are persisted in the `preferences` DB table via the API. The process view has a sortable table (c/m/p/n/t/f keys), search/filter (/), process pinning (_), and shows top N processes by CPU over time in the history chart. All system processes are visible (not just top N); non-enriched processes display `--` for FDs and cmdline columns. Pinned processes show a `_` indicator; pins are persisted via the preferences API.
...
- **Dynamic tab visibility**: temperature and power tabs are hidden when no data is available from those collectors (either disabled in config or no hardware detected). The `Model.visibleTabs` slice tracks which tabs are shown; `updateVisibleTabs()` rebuilds this on startup and each tick. When tabs are hidden, remaining tabs renumber (e.g., if Power is hidden, Procs shifts to its key). Key handlers and `renderTabBar()` use `visibleTabs` for dynamic mapping.
...
- **Notification methods**: the `Notifier` interface in `internal/alert/notifier.go` abstracts all notification delivery. Each type implements `Name()`, `Method()`, and `Send(*Alert) NotifyResult`. The engine constructs `[]Notifier` from config at startup. `sendNotifications()` is async fire-and-forget; `SendTestNotifications()` is synchronous for the test endpoint. Supported methods: **webhook** (HTTP POST with custom headers), **ntfy** (push via ntfy.sh or self-hosted, severity→priority mapping), **email** (SMTP with STARTTLS/implicit TLS, `net/smtp` stdlib), **gotify** (self-hosted push server, `X-Gotify-Key` auth), **command** (arbitrary shell command with alert details as `BEWITCH_*` env vars, 10s timeout). Config: `[[alerts.webhooks]]`, `[[alerts.ntfy]]`, `[[alerts.email]]`, `[[alerts.gotify]]`, `[[alerts.commands]]`. Test endpoint: `POST /api/test-notifications` (also aliased at `/api/test-webhook`). TUI: `t` key on alerts view tests all configured notifiers.
